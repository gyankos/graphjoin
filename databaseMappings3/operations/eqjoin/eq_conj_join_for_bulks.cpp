...
double EqConjunctiveJoinForBulks(const std::string &leftPath, const std::string &rightPath, const std::string &resultPath,
    ...
    //std::vector<hashstruct> hashes_with_offset; // it is ordered because the hash file is ordered, and the search is ordered, too
    {
        ...
        int fdLeft2 = 0;
        unsigned long sizeLeft2 = 0;
        INDEXFILE *indexLeft = (INDEXFILE*)mmapFile(leftPath+"_Index.bin",&sizeLeft2,&fdLeft2);

        std::map<unsigned long, unsigned long> id_to_offset;
        // Differently from the other implementation, the bulk graph (right) could have the vertices non serialized from
        // 0 to n-1, and hence we need to load that information into secondary memory;
        {
            int fdRight2 = 0;
            unsigned long sizeRight2 = 0;
            INDEXFILE *indexRight = (INDEXFILE*)mmapFile(rightPath+"_Index.bin",&sizeRight2,&fdRight2);
            unsigned long n = sizeRight2 / sizeof(INDEXFILE);
            for (unsigned long i = 0; i<n; i++) {
                id_to_offset.emplace(indexRight[i].id, indexRight[i].offset);
            }
            close(fdRight2);
        }
                                    ...
                                    if (hL< hR) {
                                        ...
                                        while (ii<NuSize && Nu[ii].adjacentHash == hL) {
                                            ...
                                            while (jj<NvSize && Nv[jj].adjacentHash == hR) {
                                                VAEntry vp;
                                                descriptorFromOffset(&vp,id_to_offset[Nv[jj].adjacentID],fileOffsetRight);
